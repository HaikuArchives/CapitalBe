...
void Category::SetNameAt(const int32 &index, const char *string)
{
...
void Category::SetAmountAt(const int32 &index, const Fixed &fixed)
{
	CatItem *item = fList->ItemAt(index);
...
void Category::SetMemoAt(const int32 &index, const char *string)
{
	CatItem *item = fList->ItemAt(index);
...
const char *Category::NameAt(const int32 &index) const
{
	CatItem *item = fList->ItemAt(index);
...
Fixed Category::AmountAt(const int32 &index) const
{
	CatItem *item = fList->ItemAt(index);
...
const char *Category::MemoAt(const int32 &index) const
{
	CatItem *item = fList->ItemAt(index);
...
bool Category::AddItem(const char *name, const Fixed &amount, const char *memo)
{
	if(!name)
		return false;
...
bool Category::HasItem(const char *name)
{
	// Prevent duplicates
...
bool Category::RemoveItem(const char *name)
{
	if(!name)
		return false;
...
bool Category::RemoveItem(const int32 &index)
{
...
void Category::Flatten(BFile *file)
{
	...
	for(int32 i=0; i<fList->CountItems(); i++)
	{
		...
		str << "\t\t\t<CategoryEntry Name=\"" << temp 
			<< "\" Amount=\"" << data->amount.AsFixed() << "\" />\n";
	}
...
void Category::PrintToStream(void) const
{
	BString str("<CAT>  ");
...
Category &Category::operator=(const Category &from)
{
	fList->MakeEmpty();
...
void Category::MakeEmpty(void)
{
	fList->MakeEmpty();
}

void Category::Sort(void)
{
	fList->SortItems(CompareCatItem);
}

int Category::CompareCatItem(const CatItem *item1, const CatItem *item2)
{
	if(!item1)
	{
		if(item2)
