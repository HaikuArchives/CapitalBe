...
BudgetWindow::BudgetWindow(const BRect &frame)
	: BWindow(
		  frame, TRANSLATE("Budget"), B_DOCUMENT_WINDOW,
...
BudgetWindow::BuildBudgetSummary(void) {
	...
	fBudgetSummary->AddColumn(
		new BStringColumn(
			TRANSLATE("Summary"), fBudgetSummary->StringWidth(TRANSLATE("Spending")) + 20, 10, 300,
			B_TRUNCATE_END
		),
		0
	);
	...
	for (int8 i = 0; i < 12; i++) {
		...
		fBudgetSummary->AddColumn(
			new BStringColumn(
				month, fBudgetSummary->StringWidth(month) + 25, 10, 300, B_TRUNCATE_END,
				B_ALIGN_RIGHT
			),
			i + 1
		);
	...
	fBudgetSummary->AddColumn(
		new BStringColumn(
			TRANSLATE("Total"), fBudgetSummary->StringWidth(TRANSLATE("Total")) + 20, 10, 300,
...
BudgetWindow::BuildCategoryList(void) {
	...
	fCategoryList->AddColumn(
		new BStringColumn(
			TRANSLATE("Category"), fCategoryList->StringWidth(TRANSLATE("Category")) + 20, 10, 300,
			B_TRUNCATE_END
		),
		0
	);
	fCategoryList->AddColumn(
		new BStringColumn(
			TRANSLATE("Amount"), fCategoryList->StringWidth(TRANSLATE("Amount")) + 20, 10, 300,
			B_TRUNCATE_END, B_ALIGN_RIGHT
		),
		1
	);
	fCategoryList->AddColumn(
		new BStringColumn(
			TRANSLATE("Frequency"), fCategoryList->StringWidth(TRANSLATE("Frequency")) + 20, 10,
