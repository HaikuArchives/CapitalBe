...
MainWindow::MainWindow(BRect frame) : BWindow(frame, "", B_DOCUMENT_WINDOW, 0) {
	...
	menu->AddItem(new BMenuItem(
		B_TRANSLATE("Previous"), new BMessage(M_PREVIOUS_ACCOUNT), B_UP_ARROW,
		B_COMMAND_KEY | B_SHIFT_KEY
	));
	menu->AddItem(new BMenuItem(
		B_TRANSLATE("Next"), new BMessage(M_NEXT_ACCOUNT), B_DOWN_ARROW, B_COMMAND_KEY | B_SHIFT_KEY
	));
...
MainWindow::MessageReceived(BMessage *msg) {
	...
	case M_ENTER_TRANSFER: {
		if (gDatabase.CountAccounts() < 2) {
			ShowAlert(
				B_TRANSLATE("Not enough accounts for a transfer."),
	...
	case M_SCHEDULE_TRANSACTION: {
		...
		if (data.Type().TypeCode() == TRANS_NUMERIC) {
			ShowAlert(
				B_TRANSLATE("Numbered transactions cannot be scheduled."),
	...
	case M_SHOW_RECONCILE_WINDOW: {
		if (!acc) {
			if (gDatabase.CountAccounts() < 1)
				ShowAlert(
					B_TRANSLATE("Oops!"),
					B_TRANSLATE("You need to have an account created in order to reconcile it.")
				);
			else
				ShowAlert(
					B_TRANSLATE("Oops!"),
