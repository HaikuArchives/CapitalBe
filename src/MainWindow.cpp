...
MainWindow::MainWindow(BRect frame) : BWindow(frame, "", B_DOCUMENT_WINDOW, 0) {
...
#ifdef DEMO_MODE
	menu->AddItem(new BMenuItem(TRANSLATE("Buy Capital Be"), new BMessage(M_PURCHASE_FULL_VERSION))
	);
	...
	menu->AddItem(new BMenuItem(
		TRANSLATE("Previous"), new BMessage(M_PREVIOUS_ACCOUNT), B_UP_ARROW,
		B_COMMAND_KEY | B_SHIFT_KEY
	));
	menu->AddItem(new BMenuItem(
		TRANSLATE("Next"), new BMessage(M_NEXT_ACCOUNT), B_DOWN_ARROW, B_COMMAND_KEY | B_SHIFT_KEY
	));
	...
	fImportPanel = new BFilePanel(
		B_OPEN_PANEL, new BMessenger(this), NULL, B_FILE_NODE, false, new BMessage(M_IMPORT_ACCOUNT)
	);
	temp = "Capital Be: ";
	temp += TRANSLATE("Import");
	fImportPanel->Window()->SetTitle(temp.String());
	fExportPanel = new BFilePanel(
		B_SAVE_PANEL, new BMessenger(this), NULL, B_FILE_NODE, false, new BMessage(M_EXPORT_ACCOUNT)
	);
	temp = "Capital Be: ";
...
MainWindow::MessageReceived(BMessage *msg) {
	...
	case M_SET_LANGUAGE: {
		...
		if (be_roster->IsRunning("application/x-vnd.Haiku-ServicesDaemon")) {
			...
			ShowAlert(
				TRANSLATE("Changes will be made on restart"),
	...
	case M_DELETE_ACCOUNT: {
		...
		DAlert *alert = new DAlert(
			TRANSLATE("Really Delete Account?"),
	...
	case M_ENTER_TRANSFER: {
		if (gDatabase.CountAccounts() < 2) {
			ShowAlert(
				TRANSLATE("Not enough accounts for a transfer."),
	...
	case M_SCHEDULE_TRANSACTION: {
		...
		if (data.Type().TypeCode() == TRANS_NUMERIC) {
			ShowAlert(
				TRANSLATE("Numbered transactions cannot be scheduled."),
...
MainWindow::CreateTransfer(BMessage *msg) {
	...
	gDatabase.AddTransaction(
		from->GetID(), transid, date, type, payee.String(), fixed.InvertAsCopy(), "Transfer",
		memo.String()
	);
