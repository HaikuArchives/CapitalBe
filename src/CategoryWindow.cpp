...
CategoryView::CategoryView(const char *name, const int32 &flags) : BView(name, flags) {
	...
	// the category list
	fListView = new BOutlineListView(
...
CategoryView::MessageReceived(BMessage *msg) {
	...
	case M_ADD_CATEGORY: {
		...
		if (name.ICompare(TRANSLATE("Income")) == 0 || name.ICompare(TRANSLATE("Spending")) == 0 ||
			...
			ShowAlert(
				TRANSLATE("Can't use this category name"),
...
CategoryWindow::CategoryWindow(const BRect &frame)
	: BWindow(
		  frame, TRANSLATE("Categories"), B_DOCUMENT_WINDOW_LOOK, B_NORMAL_WINDOW_FEEL,
...
CategoryInputWindow::CategoryInputWindow(const BRect &frame, BView *target)
	: BWindow(
		  frame, TRANSLATE("Add Category"), B_FLOATING_WINDOW_LOOK, B_MODAL_APP_WINDOW_FEEL,
...
CategoryRemoveWindow::CategoryRemoveWindow(const BRect &frame, const char *from, BView *target)
	: BWindow(
		  frame, TRANSLATE("Remove Category"), B_FLOATING_WINDOW_LOOK, B_MODAL_APP_WINDOW_FEEL,
...
CategoryEditWindow::CategoryEditWindow(const BRect &frame, const char *oldname, BView *target)
	: BWindow(
		  frame, TRANSLATE("Edit Category"), B_FLOATING_WINDOW_LOOK, B_MODAL_APP_WINDOW_FEEL,
