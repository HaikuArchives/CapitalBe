...
void ReportWindow::ComputeCashFlow(void)
{
...
*/
	...
	BColumn *col = new BStringColumn(TRANSLATE("Category"),fGridView->StringWidth(longestname.String())+20,10,300,B_TRUNCATE_END);
	...
	for(i = 0; i<timelist.CountItems()-1; i++)
	{
		...
		col = new BStringColumn(columntitle,75,10,300,B_TRUNCATE_END);
...
/*
		...
		at the data in order to calculate a trend and also be able to look at quite a bit of data
		for an extended period of time, which the report view doesn't do a very good job of showing.
