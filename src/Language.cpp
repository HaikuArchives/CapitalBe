...
Language::Language(const entry_ref &ref)
 :	fInit(false),
 	fName(ref.name),
 	fFileRef(ref)
{
...
BString Language::Translate(const char *instring)
{
	if(!instring)
		return NULL;
...
LanguageLogger::LanguageLogger(const entry_ref &ref)
 :	Language(ref),
	fFile(&ref,B_CREATE_FILE | B_ERASE_FILE | B_READ_WRITE)

{
	SetInitialized(true);
}

LanguageLogger::~LanguageLogger(void)
{
	fFile.Unset();
}

void LanguageLogger::Initialize(void)
{
}

BString LanguageLogger::Translate(const char *instring)
{
	if(!instring)
		return BString();
