...
ScheduleListView::ScheduleListView(const char* name, const int32& flags) : BView(name, flags)
{
	...
	fListView->AddColumn(new BStringColumn(TRANSLATE("Amount"), MAX(amountwidth, amountlabelwidth),
							 25, 300, B_ALIGN_LEFT),
		1);
	fListView->AddColumn(new BStringColumn(TRANSLATE("Payments"),
							 StringWidth(TRANSLATE("Payments")) + 20, 25, 300, B_ALIGN_LEFT),
		2);
	fListView->AddColumn(new BStringColumn(TRANSLATE("Frequency"),
							 StringWidth(TRANSLATE("Frequency")) + 20, 25, 300, B_ALIGN_LEFT),
		3);
	fListView->AddColumn(new BStringColumn(TRANSLATE("Next Payment"),
							 StringWidth(TRANSLATE("Next Payment")) + 20, 25, 300, B_ALIGN_LEFT),
		4);
...
ScheduleListWindow::ScheduleListWindow(const BRect& frame)
	: BWindow(frame, TRANSLATE("Scheduled Transactions"), B_DOCUMENT_WINDOW_LOOK,
