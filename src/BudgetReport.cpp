...
ReportWindow::_ComputeBudget()
{
	...
	for (i = 0; i < fAccountList->CountItems(); i++) {
		...
		bufSQL.format("SELECT date,amount FROM %s WHERE category = %Q AND date >= %" B_PRIdTIME
					  " AND date < %" B_PRIdTIME ";",
...
map_status
MapBudgetToSubtotal(Fixed& fixed, const int& budgetperiod, const int& subtotal, const time_t& start,
	const time_t& end)
{
