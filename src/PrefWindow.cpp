...
PrefWindow::PrefWindow(const BRect &frame)
	: BWindow(
		  frame, TRANSLATE("Options"), B_FLOATING_WINDOW_LOOK, B_MODAL_APP_WINDOW_FEEL,
...
CurrencyPrefView::CurrencyPrefView(const char *name, Locale *locale, const int32 &flags)
	...
	temp = TRANSLATE("Separator");
	temp += ":";
	fCurrencySeparatorBox = new AutoTextControl(
		"moneysep", temp.String(), fLocale.CurrencySeparator(),
		new BMessage(M_NEW_CURRENCY_SEPARATOR)
	);
	...
	temp += ":";
	fCurrencyDecimalBox = new AutoTextControl(
		"moneydecimal", temp.String(), fLocale.CurrencyDecimal(),
		new BMessage(M_NEW_CURRENCY_DECIMAL)
	);
