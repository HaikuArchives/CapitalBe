...
ReportWindow::ComputeTransactions(void)
{
	...
	col = new BStringColumn(
		B_TRANSLATE("Category"), fGridView->StringWidth("0000000000") + 20, 10, 300, B_TRUNCATE_END
	);
	...
	for (int32 subtotal_index = 0; subtotal_index < count; subtotal_index++) {
		...
		if (query.eof()) {
			...
			while (!query.eof()) {
				...
				// type
				row->SetField(
					new BStringField(DeescapeIllegalCharacters(query.getStringField(1)).String()), 2
				);
