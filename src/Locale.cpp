...
bool Locale::operator==(const Locale &other) const
{
	if(	fDateFormat != other.fDateFormat ||
			fCurrencySymbol != other.fCurrencySymbol ||
			fPrefixSymbol != other.fPrefixSymbol ||
			fCurrencySeparator != other.fCurrencySeparator ||
			fCurrencyDecimal != other.fCurrencyDecimal ||
			fDateSeparator != other.fDateSeparator ||
			fCurrencyDecimalPlace != fCurrencyDecimalPlace ||
			fUseDST != other.fUseDST )
		return false;
	return true;
}

void Locale::Flatten(BFile *file)
{
	BString str("\t<Locale>\n");
...
status_t Locale::CurrencyToString(const Fixed &amount, BString &string)
{
	string = "";
...
status_t Locale::StringToCurrency(const char *string, Fixed &amount)
{
...
status_t Locale::PremultipliedStringToCurrency(const char *string, Fixed &amount)
{
...
status_t Locale::DateToString(time_t date, BString &string)
{
	char buffer[10];
...
status_t Locale::ConstructDateStringDMY(const char *in, BString &out)
{
	...
	switch(length)
	{
		...
		case 7:
		{
			...
			else
			{
				...
				if(num2 == 0)
				{
					// Although we can't say for sure, it's unlikely that the user entered a leading zero for
					// the month.
					ddm = true;
				}
				else
				{
					...
					{
						// Getting this far means that [1-2]1[1-2] or stuff like 1/12, 2/12, 11/2, or 21/2
						// As a result, we will choose the earlier result - ddm.
						ddm = true;
					}
				}
			}
...
status_t Locale::ConstructDateStringMDY(const char *in, BString &out)
{
	// This constructs a proper date string given a list of numbers
	if(!in)
		return B_ERROR;
...
*/
	...
	switch(length)
	{
		...
		case 3: // MDD, MMD. Overlap when [1][1-2][0-9]. When there is overlap, assume the earlier date.
		case 5: // MDDYY, MMDYY
				...
				else
				{
					...
					if(num==3)
					{
						...
						else
						{
							...
							// is entering a date in the year 1930 or 1931 is relatively small, we choose mmdd
							mdyy=false;
						}
					}
					else
					{
						...
						// probably means MM/DD because of when this code is written.
						mdyy=false;
					}
				}
			}
		...
		case 6: // MDYYYY, MMDDYY
		{
			...
			// MMDDYY - 0: 0-1, 1: 0-9 (1-9 if digit 0 == 0), 2: 0-3, 3: 0-9 (0-1 if digit 2 == 3), 4-5: 0-9
			bool mmddyy=false;
			charstring[0] = in[0];
			num = atoi(charstring);
			...
			else
			{
				...
				{
					// At this point we're dealing with 01xxxx or 11xxxx. We need to see if digits 2 & 3 are a date.
					charstring[0] = in[2];
					num = atoi(charstring);
					if(num > 3)
						mmddyy = false;
					...
					if(num==0)
					{
						// It's *much* less likely that the user will type in a year with a leading zero, so choose
						// mmddyy
						mmddyy = true;
					}
					else
					{
						// 01[1-2]xxx or 11[1-2]xxx. At this point, it's not possible to tell for sure which it is,
						// but probability points to x119xx or x120xx if it's a year. Otherwise, we'll assume
						// mmddyy
						charstring[0] = in[3];
...
status_t Locale::StringToDate(const char *instring, time_t &date)
{
...
void Locale::NumberToCurrency(const Fixed &number, BString &string)
{
	string = "";
...
void ShowAlert(const char *header, const char *message, alert_type type)
{
...
void ShowBug(const char *string)
{
	...
	DAlert *alert = new DAlert("Agh! Bug!",message.String(),"Make an E-mail",
			"Save to File","Quit");
...
void Locale::SetDateFormat(const date_format &format)
{
	fDateFormat = format;
}

void Locale::SetCurrencySymbol(const char *symbol)
{
	if(symbol)
		fCurrencySymbol = symbol;
}

void Locale::SetCurrencySeparator(const char *symbol)
{
	if(symbol)
		fCurrencySeparator = symbol;
}

void Locale::SetCurrencyDecimal(const char *symbol)
{
	if(symbol)
...
void Locale::SetCurrencyDecimalPlace(const uint8 &place)
{
	fCurrencyDecimalPlace = place;
}

void Locale::SetDateSeparator(const char *symbol)
{
	if(symbol)
...
{
	return "zxcvbnm/<>?asdfghjkl;':\"qwertyuiop[]\\{}|`-=~!@#%^&*()_à¡™∞ô¶•«»–≠œ∑é®†øπ‘¬å∫∂ƒ©Ωﬁ∆◊æäñ≈ç√ßñµ…÷";
}
