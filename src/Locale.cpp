...
Locale::Locale(void) { SetDefaults(); }
		...
		if (num > 3)
			...
			if (num == 3) {
				...
				if (num == 0)
					...
					// Getting this far means that [1-2]1[1-2] or stuff like 1/12, 2/12, 11/2, or
					// 21/2 As a result, we will choose the earlier result - ddm.
					ddm = true;
				}
			}
		}
...
Locale::ConstructDateStringMDY(const char *in, BString &out) {
	...
	*/
	...
	case 6: // MDYYYY, MMDDYY
	{
		...
		// MMDDYY - 0: 0-1, 1: 0-9 (1-9 if digit 0 == 0), 2: 0-3, 3: 0-9 (0-1 if digit 2 == 3), 4-5:
		// 0-9
		bool mmddyy = false;
		charstring[0] = in[0];
		num = atoi(charstring);
		...
		if (num > 1)
			...
			if (num == 0)
				...
				// At this point we're dealing with 01xxxx or 11xxxx. We need to see if digits 2 & 3
				// are a date.
				charstring[0] = in[2];
				num = atoi(charstring);
				if (num > 3)
