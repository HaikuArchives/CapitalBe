...
ReconcileWindow::ApplyChargesAndInterest(void) {
	...
	if (strlen(fCharges->Text()) > 0 &&
		...
		TransactionData chargetrans(
			fAccount, fDate->Text(), "ATM", TRANSLATE("Bank Charge"), fCharges->Text(),
			TRANSLATE("Bank Charge"), NULL, TRANS_RECONCILED
		);
	...
	if (strlen(fInterest->Text()) > 0 &&
		...
		TransactionData interesttrans(
			fAccount, fDate->Text(), TRANSLATE("DEP"), TRANSLATE("Account Interest"),
			fInterest->Text(), TRANSLATE("Account Interest"), NULL, TRANS_RECONCILED
		);
...
ReconcileWindow::AutoReconcile(void) {
	...
	if (strlen(fCharges->Text()) > 0) {
		if (gCurrentLocale.StringToCurrency(fCharges->Text(), bankchrg) == B_OK)
			...
			ShowAlert(
				TRANSLATE("Capital Be didn't understand the amount for Bank Charges."),
	...
	if (strlen(fInterest->Text()) > 0) {
		if (gCurrentLocale.StringToCurrency(fInterest->Text(), interest) != B_OK) {
			ShowAlert(
				TRANSLATE("Capital Be didn't understand the amount for Interest Earned."),
				TRANSLATE("There may be a typo or the wrong kind of currency symbol "
						  "for this account.")
			);
			return false;
	...
	ShowAlert(
		TRANSLATE("Couldn't Quick Balance."),
