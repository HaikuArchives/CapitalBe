...
bool DStringList::AddItem(const char *string)
{
	if(!string)
		return false;
...
bool DStringList::AddItem(const char *string, int32 atIndex)
{
	if(!string)
		return false;
...
bool DStringList::AddList(DStringList *newItems)
{
	if(!newItems)
		return false;
...
bool DStringList::AddList(DStringList *newItems, int32 atIndex)
{
	if(!newItems)
		return false;
...
bool DStringList::RemoveItem(BString *item)
{
	return fList.RemoveItem(item);
}

BString *DStringList::RemoveItem(int32 index)
{
	return (BString*)fList.RemoveItem(index);
}

bool DStringList::RemoveItems(int32 index, int32 count)
{
	return (BString*)fList.RemoveItems(index,count);
}

bool DStringList::ReplaceItem(int32 index, BString *newItem)
{
	return fList.ReplaceItem(index,newItem);
}

bool DStringList::HasItem(const char *string) const
{
	if(!string)
		return false;
...
BString *DStringList::FindItem(const char *string) const
{
	if(!string)
		return NULL;
...
void DStringList::MakeEmpty(bool freemem)
{
	if(freemem)
	{
		BString *str;
...
void DStringList::PrintToStream(void)
{
	BString *str;
