...
// ---------------------------------------------------------------------------
	...
	Account *AddAccount(const char *name, const AccountType &type,
						const char *status="Open", const Locale *locale=NULL);
	...
	bool AddTransaction(const uint32 &accountid, const uint32 &id, const time_t &date, 
						...
						const char *category, const char *memo, const uint8 &status=TRANS_OPEN);
	...
	bool GetTransaction(const uint32 &transid, const uint32 &accountid, 
