...
const char *ReportGrid::RowTitle(const int32 &index)
{
...
void ReportGrid::SetRowTitle(const int32 &index, const char *title)
{
...
int32 ReportGrid::FindTitle(const char *title)
{
...
status_t ReportGrid::ValueAt(const uint32 &x, const uint32 &y, Fixed &fixed)
{
...
status_t ReportGrid::SetValue(const uint32 &x, const uint32 &y, const Fixed &fixed)
{
	if(y >= (uint32)fList.CountItems() || x >= fWidth)
		return B_ERROR;
...
void ReportGrid::AddItem(const int32 &index)
{
	if(index < 0)
...
void ReportGrid::RemoveItem(const int32 &index)
{
...
void ReportGrid::MakeEmpty(void)
{
	fList.MakeEmpty();
}
...
status_t ReportGrid::GridRecord::ValueAt(const uint32 &index, Fixed &fixed)
{
...
status_t ReportGrid::GridRecord::SetValue(const uint32 &index, const Fixed &fixed)
{
	Fixed *item = (Fixed*)fList.ItemAt(index);
