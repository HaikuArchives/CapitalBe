...
void Database::CloseAccount(Account *item)
{
	if(!item)
		return;
...
void Database::RenameAccount(Account *item, const char *name)
{
	if(item && name)
	{
		LOCK;
...
void Database::ReopenAccount(Account *item)
{
	if(!item)
		return;
...
void Database::CreateFile(const char *path)
{
	...
	DBCommand("create table accountlist (accountid int primary key, name varchar(96), "
				"type varchar(12), status varchar(30));","Database::CreateFile:create accountlist");
	DBCommand("create table accountlocale (accountid int primary key, dateformat varchar(8), "
				...
				"currencyprefix char(1));","Database::CreateFile:create accountlocale");
	DBCommand("create table memorizedlist (transactionid int);","Database::CreateFile:create memorizedlist");
	...
	DBCommand("create table budgetlist (entryid int primary key, category varchar(96), "
				"amount int, period int(2), isexpense int(1));","Database::CreateFile:create budgetlist");
	DBCommand("create table transactionlist (timestamp int primary key, transid int, "
				"category varchar(96), accountid int);","Database::CreateFile:create transactionlist");
	...
	DBCommand("create table defaultlocale (dateformat varchar(8), "
				"dateseparator char(6), currencysymbol char(6),	"
				"currencyseparator char(6), currencydecimal char(6), "
				"currencyprefix char(1));","Database::CreateFile:create defaultlocale");
	DBCommand("insert into defaultlocale values('mmddyyyy', '/', '$', ',', '.', 'true');",
				"Database::CreateFile insert defaultlocale values");
	UNLOCK;
}

status_t Database::OpenFile(const char *path)
{
	if(!path)
		return B_ERROR;
	...
	{
		UNLOCK;
		ShowAlert("Couldn't open account data.","Capital Be couldn't open your financial data. "
	...
	CppSQLite3Query query = DBQuery("select * from accountlist","Database::OpenFile:get accounts from list");
...
bool Database::ImportFile(const entry_ref &ref)
{
	LOCK;
...
bool Database::ExportFile(const entry_ref &ref)
{
	LOCK;
...
Account *Database::SetCurrentAccount(const int32 &index)
{
...
int32 Database::SetCurrentAccount(Account *account)
{
	if(!account)
	{
		LOCK;
...
Account *Database::AddAccount(const char *name, const AccountType &type, const char *status,
								const Locale *locale)
{
	if(!name || !status)
		return NULL;
...
bool Database::RemoveAccount(const int &accountid)
{
	LOCK;
	BString command;
...
void Database::RemoveAccount(Account *account)
{
	if(account)
		RemoveAccount(account->GetID());
}

Account *Database::AccountByName(const char *name)
{
	if(!name)
		return NULL;
...
Account *Database::AccountByID(const uint32 &accountid)
{
	for(int32 i=0; i<fList.CountItems(); i++)
	{
		Account *acc = fList.ItemAt(i);
...
void Database::AddBudgetEntry(const BudgetEntry &entry)
{
...
bool Database::RemoveBudgetEntry(const char *category)
{
...
bool Database::HasBudgetEntry(const char *category)
{
	if(!category)
		return false;
...
bool Database::GetBudgetEntry(const char *name, BudgetEntry &entry)
{
	if(!name)
		return false;
...
int32 Database::CountBudgetEntries(void)
{
	CppSQLite3Query query = gDatabase.DBQuery("select category from budgetlist",
...
void Database::SetAccountLocale(const uint32 &accountid, const Locale &data)
{
	LOCK;
...
Locale Database::LocaleForAccount(const uint32 &id)
{
	LOCK;
	BString command;
...
void Database::SetDefaultLocale(const Locale &data)
{
	LOCK;
	BString command;
...
Locale Database::GetDefaultLocale(void)
{
	LOCK;
	CppSQLite3Query query = DBQuery("select * from defaultlocale;",
									"Database::GetDefaultLocale");
...
bool Database::UsesDefaultLocale(const uint32 &id)
{
	LOCK;
	...
	CppSQLite3Query query = DBQuery(command.String(),
									"Database::UsesDefaultLocale");
	return query.eof();
	
	UNLOCK;
}

bool Database::AddTransaction(const uint32 &accountid, const uint32 &id, 
					const time_t &date, const TransactionType &type,
					const char *payee, const Fixed &amount,
					const char *category, const char *memo, const uint8 &status)
{
	if(!payee || !category)
		return false;
	...
	command << accountid << " values(" << timestamp << ", " << id << ", " 
			<< date << ",'" << type.Type() << "', '" 
			<< epayee << "', " << amount.AsFixed() << ", '" << ecategory;
	if(memo)
		command << "', '" << ememo << "', '";
	else
		command << "', '', ";
	...
	if(account)
	{
		if(account->CurrentTransaction()==0)
		{
			...
			command << accountid << ";";
			CppSQLite3Query query = DBQuery(command.String(),"Database::AddTransaction:get current transaction");
			if(query.eof())
			{
...
bool Database::AddTransaction(TransactionData &data, const bool &newid)
{
	if(!data.IsValid())
		return false;
	...
	if(data.CountCategories()==1)
	{
		if(newid)
			...
			AddTransaction(data.GetAccount()->GetID(), id, data.Date(), data.Type(),
							data.Payee(), data.Amount(), data.NameAt(0), data.Memo(),
							data.Status());
	}
	else
	{
		...
		for(int32 i=0; i<data.CountCategories()-1; i++)
		{
			...
			// multiple categories.
			AddTransaction(data.GetAccount()->GetID(), id, data.Date(), data.Type(),
							data.Payee(), data.AmountAt(i), data.NameAt(i), data.MemoAt(i),
							data.Status());
		}
		...
		AddTransaction(data.GetAccount()->GetID(), id, data.Date(), data.Type(),
						data.Payee(), data.AmountAt(index), data.NameAt(index), 
						data.MemoAt(index),data.Status());
	}
	UNLOCK;
	return true;
}

bool Database::RemoveTransaction(const uint32 &transid)
{
	...
	CppSQLite3Query query = DBQuery(command.String(),"Database::RemoveTransaction:find transaction");
...
uint32 Database::NextTransactionID(void)
{
	LOCK;
...
bool Database::HasTransaction(const uint32 &transid)
{
	if(transid < 0)
		return false;
...
bool Database::GetTransaction(const uint32 &transid, const uint32 &accountid,
								TransactionData &data)
{
	LOCK;
...
void Database::SetTransactionStatus(const uint32 &transid, const uint8 &status)
{
	LOCK;
	
	BString command;
	command << "select accountid from transactionlist where transid = " << transid << ";";
	CppSQLite3Query query = DBQuery(command.String(),"Database::SetTransactionStatus:get accountid");
...
bool Database::GetTransferCounterpart(const uint32 &transid, TransactionData &data)
{
	...
	CppSQLite3Query query = DBQuery(command.String(),"Database::SetTransferCounterpart:get accountid");
...
void Database::AddScheduledTransaction(const ScheduledTransData &data, const bool &newid)
{
	if(!data.IsValid())
		return;
...
//				data.SetNextDueDate(IncrementDateByMonth(data.Date()));
				ShowBug("Unimplemented Weekly scheduling support in Database::AddScheduledTransaction()");
				break;
			}
	...
	else
	{
		...
		for(int32 i=0; i<data.CountCategories()-1; i++)
		{
			...
			// multiple categories.
			InsertSchedTransaction(id, data.GetAccount()->GetID(), data.Date(), data.Type(),
									data.Payee(), data.AmountAt(i), data.NameAt(i), 
									data.MemoAt(i), data.GetInterval(), nextdate, data.GetCount());
		}
		...
		InsertSchedTransaction(id, data.GetAccount()->GetID(), data.Date(), data.Type(),
								data.Payee(), data.AmountAt(index),	data.NameAt(index),
								data.MemoAt(index), data.GetInterval(),	nextdate, data.GetCount());
	}
	UNLOCK;
}

void Database::RemoveScheduledTransaction(const uint32 &id)
{
	LOCK;
...
bool Database::GetScheduledTransaction(const uint32 &transid, ScheduledTransData &data)
{
	LOCK;
...
uint32 Database::CountScheduledTransactions(void)
{
...
bool Database::InsertSchedTransaction(const uint32 &id, const uint32 &accountid,
						const time_t &startdate, const TransactionType &type,
						const char *payee, const Fixed &amount,	const char *category, 
						const char *memo, const TransactionInterval &interval,
						const time_t &nextdate, const int32 &count)
{
	...
	command << timestamp << ", " << accountid << ", " << id << ", " 
			<< startdate << ",'" << type.Type() << "', '" 
			<< epayee << "', " << amount.AsFixed() << ", '" << ecategory;
	if(memo)
		command << "', '" << ememo << "', ";
...
int32 Database::GetLastKey(const char *table, const char *column)
{
	// Internal method. No locking required
	if(!table || !column)
		return B_ERROR;
...
void Database::AddCategory(const char *name, const bool &isexpense)
{
...
void Database::RemoveCategory(const char *name)
{
	if(!name || !HasCategory(name))
		return;
...
bool Database::RenameCategory(const char *oldname, const char *newname)
{
...
bool Database::HasCategory(const char *name)
{
	BString command;
	CppSQLite3Query query;
	...
	command << "select name from categorylist where name = '"
			<< EscapeIllegalCharacters(name) << "';";
	query = DBQuery(command.String(),"Database::HasCategory:find category");
	
	return !query.eof();
}

bool Database::IsCategoryExpense(const char *name)
{
	if(!name)
		return false;
...
void Database::SetCategoryExpense(const char *name, const bool &isexpense)
{
	if(!name || !HasCategory(name))
		return;
...
void Database::RecategorizeTransactions(const char *from, const char *to)
{
	if(!HasCategory(from))
		return;
...
void Database::DBCommand(const char *command, const char *functionname)
{
	if(!command)
...
CppSQLite3Query Database::DBQuery(const char *query, const char *functionname)
{
	if(!query)
