...
Database::CreateFile(const char *path) {
	...
	DBCommand(
		"create table memorizedlist (transactionid int);",
	...
	DBCommand(
		"create table categorylist (name varchar(96), type int(2));",
	...
	DBCommand(
		"insert into defaultlocale values('mmddyyyy', '/', '$', ',', '.', 'true');",
...
Database::AddTransaction(TransactionData &data, const bool &newid) {
	...
	if (data.CountCategories() == 1) {
		...
		for (int32 i = 0; i < data.CountCategories() - 1; i++) {
			...
			// multiple categories.
			AddTransaction(
				data.GetAccount()->GetID(), id, data.Date(), data.Type(), data.Payee(),
...
Database::AddScheduledTransaction(const ScheduledTransData &data, const bool &newid) {
	...
	if (data.CountCategories() == 1) {
		...
		for (int32 i = 0; i < data.CountCategories() - 1; i++) {
			...
			// multiple categories.
			InsertSchedTransaction(
				id, data.GetAccount()->GetID(), data.Date(), data.Type(), data.Payee(),
				data.AmountAt(i), data.NameAt(i), data.MemoAt(i), data.GetInterval(), nextdate,
				data.GetCount()
			);
		}
		...
		InsertSchedTransaction(
			id, data.GetAccount()->GetID(), data.Date(), data.Type(), data.Payee(),
			data.AmountAt(index), data.NameAt(index), data.MemoAt(index), data.GetInterval(),
